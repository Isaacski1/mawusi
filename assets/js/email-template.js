<<<<<<< HEAD
// EmailJS initialization is now handled in HTML head to ensure it's loaded before other scripts
...
function sendReservationEmail(formData) {
  const button = document.querySelector(
    '.wrap-form-booking button[type="submit"]'
  );
  ...
  const templateParams = {
    ...
    specialRequests: formData.specialRequests || "No special requests",
  ...
  emailjs
    .send("service_uvwuw6g", "template_7xa0yxb", templateParams)
...
document.getElementById("pay-now-btn").addEventListener("click", function () {
  ...
  // Generate order ID
  const orderId =
    "ORDER-" + Math.random().toString(36).substr(2, 6).toUpperCase();
...
📝 Additional Notes: ${data.message || "None"}`;
    ...
    const whatsappLink = `https://wa.me/233549175604?text=${encodeURIComponent(
      whatsappMessage
    )}`;
    ...
    alert(
      "🎉 Order submitted successfully and payment completed! Order ID: " +
        orderId +
    ...
    let errorMessage =
      "Payment successful but email failed. Order ID: " + orderId;
...
>>>>>>> 3cd0e8e6b59dccdd0b5bfb7fcbca7a5cffa6f8eb
